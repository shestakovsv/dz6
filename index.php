...
<form  method="post">
    ...
    <div id="f_metro_id"> <select title="Выберите станцию метро" name="metro_id" id="fld_metro_id" class="form-input-select"> <option value="">-- Выберите станцию метро --</option>
            ...
            <option value="2023">Студенческая</option></select> 
...
//подсчет коолличества объявлений
//$number_ads = count($_SESSION['advt']); 



//echo "<br>";
// echo 'Ваше Имя:'.$_SESSION['advt'][1]['seller_name'];
...
function title($x) {
    ?>
        <a href="index.php?id=<?php echo $x; ?>">название объявления</a> | Цена | Имя | 
        <a href="index.php?id_del=<?php echo $x; ?>">Удалить</a>
        
    <?php  
}

// completed_form_advt(1);
//$number_ads = count($_SESSION); 
//echo $number_ads;
...
//
//
//Вывод введеных объявлений
for ($index = 0; $index < count($_SESSION['advt']); $index++) {
    title($index); 
    echo "<br>";
//echo $index;
